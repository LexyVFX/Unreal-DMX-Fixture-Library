...
void ULexyVFXDMXBaseComponent::UpdateDMXMaterialScalarParameter(UMaterialInstanceDynamic * miTargetMaterial, EDMXParameterBitDepth DMXBitDepth, FName nMaterialParameterName, float fScaleFactor, float fRangeMin, float fRangeMax, TMap<FName, int32> NImapDMXFunctionValues, FName nDMXComponentFunction)
{
	...
	fScalar = fScaleFactor * FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(fRangeMin, fRangeMax), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunction));
...
void ULexyVFXDMXBaseComponent::UpdateDMXMaterialVectorParameter(UMaterialInstanceDynamic * miTargetMaterial, EDMXParameterBitDepth DMXBitDepth, FName nMaterialParameterName, TMap<FName, int32> NImapDMXFunctionValues, TArray<FName> nDMXComponentFunctions)
{
	...
	inVector4.W = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(0.0f, 1.0f), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunctions[0]));
	inVector4.X = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(0.0f, 1.0f), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunctions[1]));
	inVector4.Y = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(0.0f, 1.0f), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunctions[2]));
	inVector4.Z = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(0.0f, 1.0f), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunctions[3]));
...
void ULexyVFXDMXBaseComponent::UpdateDMXSpringArm(EDMXParameterBitDepth DMXBitDepth, USpringArmComponent * SpringArmComponentRef, float fRange, TMap<FName, int32> NImapDMXFunctionValues, FName nDMXComponentFunction)
{
	...
	SpringArmComponentRef->TargetArmLength = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(0.0f, fRange), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunction));
}
...
bool ULexyVFXDMXBaseComponent::UpdateDMXSpotConeAngle(EDMXParameterBitDepth DMXBitDepth, ULightComponent * LightComponentRef, float fBeamRangeMax, float fBeamRangeMin, TMap<FName, int32> NImapDMXFunctionValues, FName nDMXComponentFunction)
{
	...
	outFloat = 0.7f * FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(fBeamRangeMax, fBeamRangeMin), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunction));
...
void ULexyVFXDMXBaseComponent::UpdateDMXLightIntensity(EDMXParameterBitDepth DMXBitDepth, ULightComponent * LightComponentRef, float fRange, TMap<FName, int32> NImapDMXFunctionValues, FName nDMXComponentFunction)
{
	...
	LightComponentRef->Intensity = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(0.0f, fRange), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunction));
}
...
bool ULexyVFXDMXBaseComponent::UpdateDMXRotation(EDMXParameterBitDepth DMXBitDepth, USceneComponent * SceneComponentRef, EDMXRotationMode eRotationMode, float fRange, TMap<FName, int32> NImapDMXFunctionValues, FName nDMXComponentFunction)
{
	...
	outFloat = FMath::GetMappedRangeValueClamped(FVector2D(0.0f, fMaxParameterRange), FVector2D(fRange * -0.5f, fRange * 0.5f), (float)NImapDMXFunctionValues.FindRef(nDMXComponentFunction));
